...
model_dataset.treat <- model_dataset %>% mutate(treat = 1)
model_dataset.untreat <- model_dataset %>% mutate(treat = 0)
...
ATE_standertized_logistic_model <- mean(pred_treat) - mean(pred_untreat)
...
glm_ATE_est_interval <- ATE_standertized_logistic_model + c(-1, 1)*qnorm(1-(alpha/2))*glm_sd_boot
...
#### ATE/ ATT Estimator
+ ATT_Estimator : **__`r ATE_standertized_logistic_model`__**
